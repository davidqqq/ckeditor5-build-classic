		...
		editor.conversion.for('upcast')
			.add(upcastElementToElement( {
				...
				model: ( viewElement, modelWriter ) => {
					...
					const codeBlock = modelWriter.createElement( 'codeBlock')
					// modelWriter.setAttribute("language",codeElement.getAttribute('class'),codeBlock)
					modelWriter.setAttribute("language","javascript hljs",codeBlock)
					return codeBlock 
				}
			} ));
...
export function modelCodeBlockToView() {
	return dispatcher => {
		console.log('recevied event insert:codeBlock')
		dispatcher.on('insert:codeBlock', converter.bind(this), {
			priority: 'high'
		});
	...
	function converter(evt, data, conversionApi) {
		...
		if (codeBlock.childCount === 0) {
			return;
		}
		const viewWriter = conversionApi.writer;
		// if(this.editor.LANGUAGE){
		// 	viewWriter.setAttribute('language',this.editor.LANGUAGE,codeBlock)
		// }
		// const language = (codeBlock.getAttribute('language') || '') + ' hljs';
		const language = "javascript hljs"
