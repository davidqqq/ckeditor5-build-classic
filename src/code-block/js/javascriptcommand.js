...
export default class JavascriptCommand extends Command {
	...
	_applyBlock( writer, blocks ) {
		...
		getRangesOfBlockGroups( blocks ).reverse().forEach( groupRange => {
			...
			if ( !block ) {
				writer.wrap( groupRange, new Element( 'javascriptCodeBlock' ) );
...
function findBlock( elementOrPosition ) {
	return elementOrPosition.parent.name == 'javascriptCodeBlock' ? elementOrPosition.parent : null;
}
...
function checkCanBeBlocked( schema, block ) {
	...
	const isCBAllowed = schema.checkChild( block.parent, 'javascriptCodeBlock' );
	const isBlockAllowedInCB = schema.checkChild( [ '$root', 'javascriptCodeBlock' ], block );
