...
export default class PythonCommand extends Command {
	...
	execute() {
		...
		model.change( writer => {
			if ( this.value ) {
				...
				// this.editor.LANGUAGE=prompt("Enter a language if you think you know better");
	...
	_removeBlock( writer, blocks ) {
		...
		getRangesOfBlockGroups( blocks ).reverse().forEach( groupRange => {
			...
			if ( groupRange.start.isAtStart ) {
				const positionBefore = Position._createBefore( groupRange.start.parent );
			...
			const positionAfter = Position._createAfter( groupRange.end.parent );
	...
	_applyBlock( writer, blocks ) {
		...
		getRangesOfBlockGroups( blocks ).reverse().forEach( groupRange => {
			...
			if ( !block ) {
				writer.wrap( groupRange, new Element( 'pythonCodeBlock' ) );
		...
		blocksToMerge.reverse().reduce( ( currentBlock, nextBlock ) => {
			if ( currentBlock.nextSibling == nextBlock ) {
				writer.merge( Position._createAfter( currentBlock ) );
...
function findBlock( elementOrPosition ) {
	return elementOrPosition.parent.name == 'pythonCodeBlock' ? elementOrPosition.parent : null;
}
...
function getRangesOfBlockGroups( blocks ) {
	...
	while ( i < blocks.length ) {
		...
		if ( !startPosition ) {
			startPosition = Position._createBefore( block );
		}

		if ( !nextBlock || block.nextSibling != nextBlock ) {
			ranges.push( new Range( startPosition, Position._createAfter( block ) ) );
			startPosition = null;
		}
...
function checkCanBeBlocked( schema, block ) {
	...
	const isCBAllowed = schema.checkChild( block.parent, 'pythonCodeBlock' );
	if(block.parent.name.includes('CodeBlock')){
		return false
	}
	const isBlockAllowedInCB = schema.checkChild( [ '$root', 'pythonCodeBlock' ], block );
