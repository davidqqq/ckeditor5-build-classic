...
import quoteIcon from './python.svg'
 ...
 *
 * Introduces the `'pythonCodeBlock'` command and the `'pythonCodeBlock'` model element.
 *
 * @extends module:core/plugin~Plugin
 */
export default class PythonEditing extends Plugin {
	/**
	 * @inheritDoc
	 */
	init() {
		...
		editor.commands.add('pythonCodeBlock', new PythonCommand(editor));

		schema.register('pythonCodeBlock', {
			isObject: true,
			isBlock: true,
			allowContentOf: '$root',
			allowWhere: ['$block'],
		...
		// Disallow pythonCodeBlock in pythonCodeBlock.
		schema.addChildCheck((ctx, childDef) => {
			
			if (ctx.endsWith('pythonCodeBlock') && childDef.name == 'pythonCodeBlock') {
				return false;
			}
		});
		...
		editor.conversion.for('upcast')
			.add(upcastElementToElement( {
				...
				model: ( viewElement, modelWriter ) => {
					...
					const pythonCodeBlock = modelWriter.createElement( 'pythonCodeBlock')
					modelWriter.setAttribute("language",'python hljs',pythonCodeBlock)
					return pythonCodeBlock 
				}
			} ));

		editor.ui.componentFactory.add('pythonCodeBlock', locale => {
			const command = editor.commands.get('pythonCodeBlock');
			...
			this.listenTo(buttonView, 'execute', () => {
				...
				editor.execute('pythonCodeBlock')
			});
	...
	afterInit() {
		...
		this.listenTo(this.editor.editing.view.document, 'enter', (evt, data) => {
			...
			if (doc.selection.isCollapsed && positionParent.isEmpty &&
				...
				this.editor.execute('pythonCodeBlock');
...
export function modelCodeBlockToView() {
	return dispatcher => {
		console.log('recevied event insert:pythonCodeBlock')
		dispatcher.on('insert:pythonCodeBlock', converter.bind(this), {
			priority: 'high'
		});
	...
	function converter(evt, data, conversionApi) {
		const pythonCodeBlock = data.item;
		
		// Consume the codeblock and texts
		conversionApi.consumable.consume(pythonCodeBlock, 'insert')
		// We require the codeblock to have a single text node.
		if (pythonCodeBlock.childCount === 0) {
			return;
		}
		...
		conversionApi.mapper.bindElements(pythonCodeBlock, codeElement);
